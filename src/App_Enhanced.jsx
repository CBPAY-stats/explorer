...
function App() {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return allTransactions.filter(transaction => {
      ...
      if (filters.type && filters.type !== 'all') {
        ...
        const hasMatchingType = operations.some(op => op.type === filters.type)
  ...
  const getOperationDetails = (transaction) => {
    const operations = transaction.operations || []
    if (operations.length === 0) return { type: 'Unknown', from: '', to: '', amount: '', asset: 'XDB' }
    ...
    switch (mainOperation.type) {
      case 'payment':
        type = mainOperation.to === walletAddress ? 'Received' : 'Sent'
    ...
    return { type, from, to, amount, asset, operationCount: operations.length }
  }
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-start justify-between mb-3">
                              <div className="flex items-center gap-2">
                                <Badge 
                                  variant={details.type === 'Received' ? 'default' : details.type === 'Sent' ? 'destructive' : 'secondary'}
                                  className="text-xs"
                                >
                                  {details.type}
                                </Badge>
                            ...
                            <div className="flex items-center justify-between text-sm text-muted-foreground">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => window.open(`https://explorer.xdbchain.com/tx/${transaction.hash}`, '_blank')}
                                  className="text-xs"
                                >
